...
const GroupList = () => {
  ...
  const loadGroups = async () => {
    try {
      ...
      if (Array.isArray(response)) {
        ...
        groupData = response.data || response.content || response.groups || [];
      }
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', py: 0.5 }}>
          {params.value && Array.isArray(params.value) ? (
            <Chip
              label={`${params.value.length} users`}
              size="small"
              variant="outlined"
              color="primary"
              sx={{ fontSize: '0.75rem' }}
  ...
  const handleExportCSV = () => {
    exportToCSV(filteredGroups, 'groups');
  ...
  const handleExportPDF = () => {
    exportToPDF(filteredGroups, 'groups', t);
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Manage groups and user assignments
        </Typography>
      </Box>
