...
const FolderList = () => {
  ...
  const columns: GridColDef[] = [
    ...
    { 
      field: 'description', 
      headerName: t('folder.description') || 'Description', 
      minWidth: 250,
      flex: 2,
    },
    { 
      ...
      renderCell: (params) => (
        params.row.archiveBox ? (
          <Chip 
            label={params.row.archiveBox.code} 
            size="small" 
            color="primary"
            variant="outlined"
          />
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          ...
          <Stack direction="row" spacing={1.5}>
            ...
            >
              {t('folder.create') || 'Create Folder'}
            </Button>
          </Stack>
        </Box>
        <Typography variant="body2" color="text.secondary">
          Manage folders inside archive boxes
        </Typography>
      </Box>
          ...
          <Stack spacing={2.5}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              <FormControl sx={{ minWidth: 250 }} size="medium">
                ...
                <Select
                  value={selectedArchiveBoxId}
                  label={t('folder.filterByArchiveBox') || 'Filter by Archive Box'}
                  onChange={(e) => setSelectedArchiveBoxId(e.target.value as number | '')}
                >
                  <MenuItem value="">
